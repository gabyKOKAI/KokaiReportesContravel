...
class ComisionesContravel:
	...
	def readCsvIcaav(self):
		...
		try:
			...
			for line in file1:
				...
				icaavTodo[idMov]['idCliente'] = idCliente
			...
			if(renglon["idCliente"]in self.dicExcepciones or renglon["serieFac"]=="NCE"):
				...
				if(renglon["lineaAerea"] == "AM"):
					if(renglon["clases"]==['']):
						...
						if(renglon["interlineal"] == False and "V" not in renglon["clases1"] and renglon["paisOrigen"] == "MEX"):
					...
					if(renglon["clases"]==['']):
						...
						if(renglon["ruta"][0] == "CUN"):
							 res = self.regresaComision(renglon)
						if((renglon["ruta"][0] == "MIA" or renglon["ruta"][0] == "JFK") and renglon["paisDestino"] == "MEX"):
							res = self.regresaComision(renglon)
						if(renglon["interlineal"] == True):
							res = "0%"
						if(renglon["codigoComp"] == True):
	...
	def readIATACodes(self):
		...
		try:
			...
			try:
				...
				self.addMensaje("error", "El archivo " + src  + " no existe.")
	...
	def readExcepciones(self):
		...
		try:
			...
			for line in file1:
				self.dicExcepciones[line.split(',')[0]+line.split(',')[1]] = line.split(',')[2]		
		...
		try:
			...
			with open(dst, 'w') as csv_file:
				...
				for idMov in icaavTodo:
					...
					icaavTodo[idMov]["paisOrigen"] = self.regresaPais(icaavTodo[idMov]["ruta"][0])
					csv_file.write(icaavTodo[idMov]["paisOrigen"] + ",")
					csv_file.write(icaavTodo[idMov]["ruta"][0] + ",")
	
					tamRuta = len(icaavTodo[idMov]["ruta"])
					icaavTodo[idMov]["paisDestino"] = self.regresaPais(icaavTodo[idMov]["ruta"][tamRuta-1])
					csv_file.write(icaavTodo[idMov]["paisDestino"] + ",")
					csv_file.write(icaavTodo[idMov]["ruta"][tamRuta-1] + ",")
