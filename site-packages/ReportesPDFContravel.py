...
class ReportesPDFContravel:
    ...
    def createArribaTitReporte(self, nomClienteTit):
        ...
        try:
            ...
            self.c.setFont('Courier-Bold', 12)
            self.c.drawString(25, self.renglon, "%-*s%*s%+*s" % (14, "Reporte de " + self.reporteType,
                                                       68, self.periodo.center(60, ' '),
                                                       16, "Página: " + str(self.linePage)))
    ...
    def createTitle(self, nomClienteTit):
        ...
        try:
            ...
            elif self.reporteType == "Comisiones":
    ...
    def createSumaryVentas(self):
        ...
        try:
            ...
            self.c.drawString(75,self.renglon, str("¡¡¡NO OLVIDE CAPTURAR LA REFERENCIA BANCARIA!!! : " + self.clienteAnterior).center(70)[:70])
            self.renglon = self.renglon - 20
            self.c.drawString(30,self.renglon, "%*s"%(45,"¡Muchas Gracias!"))
    ...
    def createSumaryComisiones(self):
        ...
        try:
            ...
            ##self.renglon = self.renglon - 26
            self.c.setFillColorRGB(255,0,0)
            self.c.drawString(30,self.renglon, "¡¡¡Favor de NO ELABORAR FACTURAS de comisiones los ÚLTIMOS 2 DIAS DEL MES!!!")
            self.renglon = self.renglon - 20
            self.c.drawString(30,self.renglon, "%*s"%(45,"¡Muchas Gracias!"))
    ...
    def createReport(self):
        ...
        try:
            ...
            if not ".csv" in src:
                ...
                self.mensajesErr[tam]["mensaje"] = "Recuerda que debes guardarlo como archivo CSV."
                self.mensajesErr[tam]["tipo"] = "error"
            else:
                ...
                for line in file1:
                    ...
                    ##print(numBoleto)
                    ##print(line.split(',')[0])
                    self.cliente = line.split(',')[0]
                    self.nomCliente = line.split(',')[1][:60] + "-" + self.cliente
                    ##print(self.nomCliente)
                    if (self.cliente not in self.clienteAnterior):
                        ...
                        if not os.path.exists(self.directorio + "ReportesS" + self.semana + self.ano + "\\"):
                            os.mkdir(self.directorio + "ReportesS" + self.semana + self.ano + "\\")
                        dst = self.directorio + "ReportesS" + self.semana + self.ano + "\\" + self.reporteType[0] + self.cliente + self.semana + self.ano + ".pdf"
            ...
            tam = len(self.mensajesErr)
            self.mensajesErr[tam] = {}
            self.mensajesErr[tam]["mensaje"] = "Verifica en el boleto:" + numBoleto + " y la linea: " + linea + " o en el siguiente."
            self.mensajesErr[tam]["tipo"] = "warning"

            tam = len(self.mensajesErr)
            self.mensajesErr[tam] = {}
            self.mensajesErr[tam]["mensaje"] = "Recuerda que: Las cantidades deben tener formato General. El documento no deben tener titulos ni deben existir comas ´,´ en el archivo CSV antes de guardarlo"
            self.mensajesErr[tam]["tipo"] = "info"

            tam = len(self.mensajesErr)
            self.mensajesErr[tam] = {}
            self.mensajesErr[tam]["mensaje"] = "Error sistema: " + str(err)
            self.mensajesErr[tam]["tipo"] = "error"
    ...
    def createZip(self):
        ...
        try:
            res = ""
            ##servidor
            shutil.make_archive(self.directorio + "ReportesS" + self.semana + self.ano, 'zip', self.directorio + "ReportesS" + self.semana + self.ano + "\")
